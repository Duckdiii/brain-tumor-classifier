"""Fine-tuning loop for the ``timm`` Vision Transformer classifier."""
...
def train(
    ...
    """Fine-tune a pretrained ViT and save the best-val-accuracy checkpoint.

    Uses a linear warmup for ``warmup_epochs`` followed by cosine decay, and
    clips gradients to ``gradient_clipping``, matching ``configs/vit.yaml``.
    """
    ...
    model = build_vit_model(num_classes=num_classes, model_name=model_name, pretrained=True).to(device)
